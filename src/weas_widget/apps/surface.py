...
class SurfaceBuilder(ipw.VBox):
    ...
    def __init__(self, title="Surface editor"):
        ...
        self.repeat_x = ipw.IntText(description="Repeat x", value=1)
        self.repeat_y = ipw.IntText(description="Repeat y", value=1)
    ...
    def _update_adsorbate_sites_preview(self, atoms):
        ...
        try:
            sites = self._find_adsorbate_sites(
                ...
                self.adsorbate_site_type.value,
            )
            ...
            return
        self._adsorbate_site_count.value = (
            f"<div><strong>Sites:</strong> {len(sites)}</div>"
        )
